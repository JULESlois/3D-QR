...
requireText(appUi, 'document.body.dataset.palette = paletteKey', 'src/app-ui.ts committed palette state')
requireText(shareState, 'const palette = document.body.dataset.palette', 'src/share-state.ts committed palette state')
forbidText(shareState, "document.querySelector<HTMLButtonElement>('[data-palette].is-active')", 'src/share-state.ts committed palette state')
forbidText(shareState, 'activePalette', 'src/share-state.ts committed palette state')
...
requireText(uiControls, "document.body.dataset.exportBusy === 'true'", 'src/ui-controls.ts')
requireText(uiControls, "busyObserver.observe(document.body, { attributes: true, attributeFilter: ['data-export-busy'] })", 'src/ui-controls.ts')
forbidText(uiControls, 'sceneButtons.some((button) => button.disabled)', 'src/ui-controls.ts')
