...
const outputDir = 'high-density-qr-smoke'
const userDataDir = '.high-density-qr-smoke-chrome'
...
async function waitForValue(send, expression, predicate, label, timeoutMs = 10_000) {
  const deadline = Date.now() + timeoutMs
  let value
...
function decodeQr(bytes, style) {
  ...
  if (decoded.data !== expectedPayload) {
    throw new Error(`${style} high-density QR decoded unexpected payload: ${JSON.stringify(decoded.data)}`)
  }
  return decoded.data
}
...
async function capture(send, style) {
  ...
  if (bytes.length < 10_000) {
    throw new Error(`${style} high-density QR screenshot is unexpectedly small (${bytes.length} bytes)`)
  }
  await writeFile(`${outputDir}/${style}-high-density-qr.png`, bytes)
  return bytes
}
...
async function selectStyle(send, style) {
  ...
  await waitForValue(
    ...
    (value) => typeof value === 'string' && value.startsWith('QR V') && !value.includes('QR ERROR'),
    `${style} high-density QR build`,
  )
  await sleep(700)
}

async function installHighDensityPayload(send) {
  const previousMeta = await evaluateValue(send, `document.querySelector('#qr-meta')?.textContent ?? ''`)
  await evaluateValue(send, `(() => {
    const input = document.querySelector('#qr-input')
    if (!(input instanceof HTMLInputElement)) throw new Error('QR input was not found')
    input.value = ${JSON.stringify(expectedPayload)}
    input.dispatchEvent(new Event('input', { bubbles: true }))
    return input.value.length
  })()`)

  const meta = await waitForValue(
    send,
    `document.querySelector('#qr-meta')?.textContent ?? ''`,
    (value) => typeof value === 'string' && value !== previousMeta && value.startsWith('QR V'),
    'High-density QR rebuild',
    12_000,
  )
  if (meta.includes('QR ERROR')) throw new Error(`High-density payload failed to build: ${meta}`)

  const match = /^QR V(\d+)\b/.exec(meta)
  if (!match) throw new Error(`Could not parse QR version from status: ${meta}`)
  const version = Number(match[1])
  if (version < minimumExpectedVersion) {
    throw new Error(`High-density payload only produced QR V${version}; expected V${minimumExpectedVersion}+`)
  }
  return version
}
...
try {
  ...
  const version = await installHighDensityPayload(send)
  ...
  if (failures.length > 0) {
    throw new Error(`High-density QR decode failed for ${failures.length}/${styles.length} scenes: ${failures.join(' | ')}`)
  }

  console.log(`high-density QR smoke: V${version} / ${styles.length} scenes decoded / ${results.join(' | ')}`)
} finally {
  socket?.close()
