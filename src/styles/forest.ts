...
function isClearingCell(cell: Pick<QRCell, 'row' | 'col'>, center: number, size: number): boolean {
  if (Math.abs(cell.row - center) > size * 0.34) return false
  return Math.abs(cell.col - clearingColumn(cell.row, center)) <= 1.15
}
...
function findAnchorCell(
  ...
  return matrix.cells
    ...
    .sort((a, b) => (
      Math.hypot(a.row - targetRow, a.col - targetCol)
      - Math.hypot(b.row - targetRow, b.col - targetCol)
    ))[0]
}
...
function createTrees(matrix: QRMatrixData, seedText: string, center: number): ForestTree[] {
  ...
  for (let index = 0; index < slotLimit; index += 1) {
    ...
    const targetRow = center + slot.row * matrix.size
    const targetCol = center + slot.col * matrix.size
    const cell = findAnchorCell(matrix, targetRow, targetCol, center, used, minSpacing)
    if (!cell) continue
    ...
    const trunkHeight = slot.species === 'ancient'
      ...
      : slot.species === 'broadleaf'
        ? 5 + Math.floor(noise * 2)
        : 6 + Math.floor(noise * 2)
    const crownHeight = slot.species === 'ancient'
      ? 8 + Math.floor(noise * 3)
      : slot.species === 'broadleaf'
        ? 6 + Math.floor(noise * 3)
  ...
  for (let dr = -extent; dr <= extent; dr += 1) {
    for (let dc = -extent; dc <= extent; dc += 1) {
      ...
      if (tree.species === 'pine') {
        ...
        topLevel = tree.trunkHeight + 2 + Math.round(cone * tree.crownHeight + noise * 1.4)
        fromLevel = Math.max(3, tree.trunkHeight - 2 + Math.round(radial * 2))
      } else {
        const dome = Math.sqrt(Math.max(0, 1 - radial * radial))
        const ancientBoost = tree.species === 'ancient' ? 2 : 0
        topLevel = tree.trunkHeight + 2 + ancientBoost + Math.round(dome * tree.crownHeight + noise * 1.8)
        const thickness = tree.species === 'ancient' ? 5 : 4
        fromLevel = Math.max(3, topLevel - thickness - Math.round(dome * 1.5))
      }
...
function buildForestPath(
  ...
  for (let row = startRow; row <= endRow; row += 1) {
    ...
    for (const col of [pathCol - 1, pathCol]) {
      ...
      if (!cell || cell.zone === 'finder') continue
...
export function generateForest(matrix: QRMatrixData, seedText: string): SculptureBuild {
  ...
  return finalizeSculpture(
    ...
    'Forest',
    lifted,
    `${trees.length} TREES / ${speciesCounts.broadleaf} BROADLEAF + ${speciesCounts.pine} PINE + ${speciesCounts.ancient} ANCIENT / WINDING CLEARING`,
    'full-pad',
  )
}
